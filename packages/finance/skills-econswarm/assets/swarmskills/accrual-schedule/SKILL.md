---
name_en: accrual-schedule
description_en: Build the period-end accrual schedule — for each accrual, compute the entry, cite the support, and draft the JE. Use during month-end close; the JE is a draft for controller approval, not a posting.
name_zh: 应计费用日程表
description_zh: 构建期末应计费用日程表——为每项应计费用计算金额、引用支撑文件并起草日记账分录。用于月末结账；日记账分录为待财务主管审批的草稿，非正式过账。
---
