...
name_zh: AlphaGBM 希腊值分析
description_zh: >
  为任意期权合约或多腿组合提供希腊值仪表板。涵盖一阶希腊值（Delta、Gamma、Theta、Vega、Rho）和二阶希腊值（Charm、Vanna、Volga）。返回单个合约和组合级别的希腊值，附带情景热力图。使用场景：检查期权敏感度、管理持仓风险、理解 Theta 衰减、分析 Gamma 敞口、对冲投资组合。
  触发词："AAPL 220 call 希腊值"、"组合希腊值"、"Theta 衰减分析"、"NVDA Gamma 敞口"、"我持仓的 Delta"、"SPY 跨式 Vega 风险"
globs:
  - 'mock-data/*.json'
---
...
```json
{
  ...
  "heatmap": {
    ...
    "delta_grid": [
      [0.12, 0.15, 0.2, 0.28, 0.38, 0.5, 0.62, 0.73],
      [0.14, 0.18, 0.24, 0.32, 0.42, 0.52, 0.63, 0.74],
      [0.16, 0.2, 0.27, 0.35, 0.45, 0.55, 0.65, 0.75],
      [0.18, 0.23, 0.3, 0.38, 0.48, 0.57, 0.67, 0.76],
      [0.2, 0.25, 0.32, 0.4, 0.5, 0.59, 0.68, 0.77]
    ],
    "pnl_grid": "..."
  },
  "insights": [
