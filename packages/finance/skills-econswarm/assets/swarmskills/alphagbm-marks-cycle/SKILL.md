...
name_zh: AlphaGBM 马克斯周期
description_zh: |
  霍华德·马克斯风格的市场周期位置评分 0-100，其中 0 = 恐慌底部（全力进攻），100 = 狂热顶部（全力防守）。将 VIX（40%）+ SPY IV 排名（25%）+ Put/Call 比率（20%）+ 估值百分位（15%）整合为单一数值，并映射为进攻 vs 防守姿态。免费端点，无需认证，5分钟缓存——目标是将"我们处于周期什么位置"变成一次调用即可获取的答案。
  触发词："市场在周期中的什么位置"、"霍华德·马克斯风格周期解读"、"我应该进攻还是防守"、"这是买入周期吗"、"当前周期位置"、"马克斯周期评分"、"SPY 情绪解读"
globs:
...
This skill gives you the one number Marks's entire philosophy implies: _where
are we right now_.
