...
name_zh: AlphaGBM 盈亏模拟器
description_zh: >
  为任意单腿或多腿期权仓位提供盈亏模拟引擎。生成到期损益图、随时间变化的 P&L、情景分析（价格、IV、时间）、盈亏平衡分析和概率分布。使用场景：测试交易构想、可视化风险/回报、运行情景分析、检查盈亏平衡点、压力测试仓位。
  触发词："模拟 AAPL 牛市看涨价差 P&L"、"如果 NVDA 跌 10% 会怎样"、"P&L 图表"、"测试我的 Iron Condor"、"盈亏平衡分析"、"压力测试我的仓位"、"到期时会发生什么"
globs:
  - 'mock-data/*.json'
---
