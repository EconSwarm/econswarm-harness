...
name_zh: AlphaGBM VIX状态
description_zh: |
  当前 VIX 水平 + 五级恐惧温度计分类 + 期权卖方策略提示。将单一的 VIX 数字转化为可操作的交易指导（平静/正常/卖方最佳区间/谨慎/极度恐惧）。包含相对过去一年历史的当前百分位以及市场在各级别停留的天数。
  触发词："VIX 多少"、"VIX 水平"、"市场平静吗"、"市场恐惧指数"、"我现在应该卖期权吗"、"VIX 级别"、"VIX 策略"、"波动率环境"、"恐惧指数"、"我该买保护吗"、"现在是做 BPS 的好时机吗"
globs:
...
```json
{
  ...
  "strategy_hint": {
    "zh": "BPS 权利金变肥，积极开仓",
    "en": "BPS premiums get juicy — actively open positions"
  },
  "distribution_1y_pct": {
    "calm": 12.5,
    "normal": 45.2,
