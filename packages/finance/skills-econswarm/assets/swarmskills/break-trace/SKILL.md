---
name_en: break-trace
description_en: Root-cause a reconciliation break to its source transaction or posting — follow the audit trail from the break row back to the originating entry on each side and state what differs and why. Use after gl-recon has classified a break.
name_zh: 差异追溯
description_zh: 追溯两期之间的差异——找出什么变化了、变化了多少以及为什么变化。用于分析期间变动、盈亏驱动因素分析或解释预算与实际差异时使用。
---
