---
name_en: china-clean-data-xls
description_en: Clean and normalize A-share financial data for modeling. Adapts the original clean-data-xls skill for Chinese financial statements, CAS conventions, and A-share data formats. Triggers on "A股数据清洗", "财务数据清洗", "clean financial data China", "数据清洗", or "normalize financial statements".
name_zh: 中国数据清洗
description_zh: 清洗中国电子表格中的混乱数据——去除空格、修复大小写不一致、转换文本型数字、标准化日期、删除重复项并标记混合类型列。适配中国市场数据格式和中文编码问题。
---
