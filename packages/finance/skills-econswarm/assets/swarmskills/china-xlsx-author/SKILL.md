---
name_en: china-xlsx-author
description_en: Create professional Excel workbooks for A-share financial analysis. Adapts the original xlsx-author skill for Chinese financial modeling standards, CAS conventions, and A-share formatting. Triggers on "A股Excel模型", "财务模型制作", "create model China", "build model xlsx", "制作模型", or "Excel model [company]".
name_zh: 中国Excel生成器
description_zh: 以无界面方式生成中国风格的.xlsx文件——用于无Excel应用的自动化代理会话中。适配中国财务表格的排版和单位惯例。
---
