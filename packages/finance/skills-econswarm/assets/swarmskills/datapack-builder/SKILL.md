---
name_en: datapack-builder
description_en: Build professional financial services data packs from various sources including CIMs, offering memorandums, SEC filings, web search, or MCP servers. Extract, normalize, and standardize financial data into investment committee-ready Excel workbooks with consistent structure, proper formatting, and documented assumptions. Use for M&A due diligence, private equity analysis, investment committee materials, and standardizing financial reporting across portfolio companies. Do not use for simple financial calculations or working with already-completed data packs.
name_zh: 数据包构建器
description_zh: 为投行交易流程构建全面的数据包。整合公司信息、财务数据、行业研究和估值分析，用于交易营销和尽职调查。
---
