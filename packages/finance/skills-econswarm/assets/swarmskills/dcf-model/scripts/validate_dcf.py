        ...
        try:
            ...
            if terminal_growth is not None and wacc is not None:
                if terminal_growth >= wacc:
                    ...
                    self.info.append(f"✓ Terminal growth ({terminal_growth:.2%}) < WACC ({wacc:.2%})")
            else:
        ...
        try:
            ...
            if wacc is not None:
                if wacc < 0.05 or wacc > 0.20:
                    self.warnings.append(f"WACC ({wacc:.2%}) is outside typical range (5%-20%). Verify calculation.")
                else:
...
def main():
    ...
    try:
        ...
        error_result = {"file": excel_file, "status": "ERROR", "error": str(e)}
