---
name_en: deal-sourcing
description_en: PE deal sourcing workflow — discover target companies, check CRM for existing relationships, and draft personalized founder outreach emails. Use when sourcing new deals, prospecting companies in a sector, or reaching out to founders. Triggers on "find companies", "source deals", "draft founder email", "check if we've seen this company", or "outreach to founder".
name_zh: 交易寻源
description_zh: 识别和评估潜在的并购目标和投资机会。使用行业筛选、网络和数据库搜索来建立交易管道。
---
