---
name_en: gl-recon
description_en: Reconcile general ledger to subledger for a trade date or period — match at the position or transaction level, surface breaks, and classify each break by likely cause. Use for daily or month-end recon runs across asset classes.
name_zh: 总账对账
description_zh: 执行总账对账——将交易和余额核对到支持文件。用于月末关账、审计准备或当科目未结清时。
---
