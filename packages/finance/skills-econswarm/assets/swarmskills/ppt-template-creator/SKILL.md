---
name_en: ppt-template-creator
description_en: Creates self-contained PPT template SKILLS (not presentations) from user-provided PowerPoint templates. Use ONLY when a user wants to create a reusable skill from their template. For creating actual presentations, use the pptx skill instead.
name_zh: PPT模板创建器
description_zh: 创建符合行业标准的金融机构PPT模板。按品牌设置幻灯片母版、布局、配色方案和排版。
---
