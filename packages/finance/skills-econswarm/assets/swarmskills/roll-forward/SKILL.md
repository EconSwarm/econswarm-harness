---
name_en: roll-forward
description_en: Build a roll-forward schedule for a balance-sheet account — beginning balance plus activity less reversals equals ending balance, with each component tied to GL. Use for month-end close packages and audit support.
name_zh: 滚动前推
description_zh: 对基金会计科目执行滚动前推——从前一期间到当前期间，验证每个科目的期初→变动→期末逻辑。用于基金管理和月末结账。
---
