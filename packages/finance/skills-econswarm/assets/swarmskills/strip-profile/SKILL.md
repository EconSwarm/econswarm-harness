---
name_en: fsi-strip-profile
description_en: |
  For potential transactions, build a simplified financial profile of the company/assets. Extract key metrics, market data, and high-level valuation parameters. Used in the initial stage of the trading workflow.
name_zh: 剥离分析
description_zh: 为潜在交易构建公司/资产的精简财务画像。提取关键指标、市场数据和高层次估值参数。用于交易流程的初始阶段。
---
...
- **Bullets for ALL body text** - NEVER paragraphs. **Use ONE textbox per section with all bullets inside** - do NOT create separate textboxes for each bullet point. Use PptxGenJS bullet formatting:
  ...
  slide.addText(
    [
      ...
      {
        text: 'Employees: 140,000+ globally across 6 continents',
        options: { bullet: { indent: 10 }, breakLine: true },
      },
      {
        text: 'CEO: Elon Musk; CFO: Vaibhav Taneja',
        options: { bullet: { indent: 10 }, breakLine: true },
      },
      {
        text: 'Market Cap: $850B (#6 globally by market cap)',
        options: { bullet: { indent: 10 }, breakLine: true },
      },
      {
        text: 'Segments: Automotive (85%), Energy (10%), Services (5%)',
...
slide.addChart(
  ...
  ],
  {
    ...
    y: 4.1,
    w: 4.5,
    h: 3.0, // Fits in bottom-right quadrant (4:3)
    barDir: 'bar',
    chartColors: ['FF6B35'],
    showValue: true,
...
slide.addChart(
  ...
  ],
  {
    ...
    y: 1.2,
    w: 9.4,
    h: 5.5, // Full width for 4:3 slide
    chartColors: ['FF6B35'],
    showValue: true,
    lineSmooth: true,
...
slide.addTable(
  ...
  ],
  {
    ...
    y: 4.1,
    w: 4.3,
    h: 3.0, // Below header in bottom-left quadrant
    fontFace: 'Arial',
    fontSize: 10,
